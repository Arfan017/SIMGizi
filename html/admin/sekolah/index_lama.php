...
// Prepare monthly aggregates for current year (jumlah_habis and jumlah_kembali)
$currentYear = (int) date('Y');
$habis_by_month = array_fill(1, 12, 0);
$kembali_by_month = array_fill(1, 12, 0);

$sql_month = "SELECT MONTH(tanggal) AS m, COALESCE(SUM(jumlah_habis),0) AS habis, COALESCE(SUM(jumlah_kembali),0) AS kembali \
FROM tb_distribusi WHERE id_sekolah_tujuan = ? AND YEAR(tanggal) = ? GROUP BY m ORDER BY m";
if ($stmt = mysqli_prepare($conn, $sql_month)) {
  $idSek = (int) $id_sekolah;
  mysqli_stmt_bind_param($stmt, 'ii', $idSek, $currentYear);
  if (mysqli_stmt_execute($stmt)) {
    ...
    if ($res) {
      while ($r = mysqli_fetch_assoc($res)) {
        $m = (int) $r['m'];
        if ($m >= 1 && $m <= 12) {
          $habis_by_month[$m] = (int) $r['habis'];
          $kembali_by_month[$m] = (int) $r['kembali'];
        }
      }
                            ...
                            if (mysqli_num_rows($result) > 0) {
                              while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                if ($row['status_pengiriman'] == '0') {
                                  $status_pengiriman_badge .= ' bg-label-secondary';
