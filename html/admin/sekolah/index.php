                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y d-flex flex-column h-100 pb-0">
                        ...
                        <div class="row gy-3">
                            ...
                            <!-- Data Distribusi per Bulan (Chart) -->
                            <div class="col-md">
                                <div class="card overflow-hidden">
                                    ...
                                    <div class="card-body p-4">
                                        <?php
                                        // Prepare monthly aggregation for the current year, starting from January
                                        $months = [];
                                        $habis_counts = [];  // Data series untuk jumlah_habis
                                        $kembali_counts = []; // Data series untuk jumlah_kembali

                                        // 1. Mulai dari Januari tahun ini
                                        $start = new DateTime('first day of January this year');
                                        $startDate = $start->format('Y-m-d');

                                        $map = [];
                                        // 2. Query diubah untuk SUM(jumlah_habis) dan SUM(jumlah_kembali)
                                        ...
                                        if ($stmt = mysqli_prepare($conn, $sql)) {
                                            $schoolInt = (int) $id_sekolah;
                                            mysqli_stmt_bind_param($stmt, 'is', $schoolInt, $startDate);
                                            mysqli_stmt_execute($stmt);
                                            $res = mysqli_stmt_get_result($stmt);
                                            if ($res) {
                                                while ($r = mysqli_fetch_assoc($res)) {
                                                    // Simpan kedua nilai dalam satu map
                                                    $map[$r['ym']] = [
                                                        'habis' => (int) $r['total_habis'],
                                                        'kembali' => (int) $r['total_kembali']
                                                    ];
                                                }
                                        ...
                                        // Loop dari Januari hingga bulan saat ini untuk mengisi data
                                        $cur = new DateTime('first day of January this year');
                                        $end = new DateTime('first day of next month'); // Batas loop adalah awal bulan depan

                                        while ($cur < $end) {
                                            $ym = $cur->format('Y-m');
                                            $months[] = $cur->format('M Y'); // Format label (e.g., "Jan 2025")

                                            if (isset($map[$ym])) {
                                                // Jika ada data untuk bulan ini, ambil nilainya
                                                $habis_counts[] = $map[$ym]['habis'];
                                                $kembali_counts[] = $map[$ym]['kembali'];
                                            } else {
                                        ...
                                        ?>
                                        <div id="chartDistribusi" style="height: 450px;"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
